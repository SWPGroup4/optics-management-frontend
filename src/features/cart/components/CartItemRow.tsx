...
export const CartItemRow = ({ 
  ...
  const hasManualInput = !!(
    item.prescription?.od?.sphere || item.prescription?.od?.cylinder || item.prescription?.od?.axis || item.prescription?.od?.add ||
    item.prescription?.os?.sphere || item.prescription?.os?.cylinder || item.prescription?.os?.axis || item.prescription?.os?.add
  );
  ...
  return (
    ...
    <div className={`flex flex-col bg-white border rounded-2xl overflow-hidden transition-all duration-300 group/item ${
      showRx ? 'border-zinc-300 shadow-md' : 'border-zinc-200 shadow-sm hover:border-zinc-300 hover:shadow-md'
    }`}>
      ...
      <div className="p-3.5 flex gap-4 bg-white relative z-10">
        ...
        <div className="w-24 h-24 bg-zinc-50/80 rounded-xl border border-zinc-100 shrink-0 relative flex items-center justify-center p-2">
          <img src={item.image} alt={item.name} className="w-full h-full object-contain mix-blend-multiply group-hover/item:scale-105 transition-transform duration-500" />
        ...
        <div className="flex-1 flex flex-col justify-between py-0.5">
          ...
          <div className="flex justify-between items-end mt-2.5">
            <div className="flex items-center bg-zinc-50 rounded-full p-0.5 border border-zinc-200">
              ...
              <span className="w-7 text-center text-xs font-black text-zinc-900">{item.quantity}</span>
              <button 
      ...
      <div className={`grid transition-all duration-300 ease-in-out ${showRx ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'}`}>
              ...
              <div className="flex flex-col gap-3">
                  ...
                  {item.prescription?.notes && (
                    <div className="flex flex-col gap-1.5 mt-1">
                      <span className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest pl-1">Ghi chú</span>
