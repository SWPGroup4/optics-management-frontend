...
export const CartDrawer = () => {
  ...
  return (
    <Sheet open={isOpen} onOpenChange={closeCart}>
      <SheetContent side="right" className="w-full sm:max-w-md p-0 flex flex-col gap-0 border-l shadow-2xl [&>button]:hidden bg-[#F4F4F5]">
        {/* HEADER */}
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-thin scrollbar-thumb-zinc-300">
          {items.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-center p-10 bg-white rounded-3xl m-2 border border-zinc-100">
              ...
              <h4 className="text-lg font-black text-zinc-900 tracking-tight mb-2 uppercase">Giỏ hàng trống</h4>
              <p className="text-sm text-zinc-500 mb-6">Có vẻ như bạn chưa thêm sản phẩm nào.</p>
              <SheetClose asChild>
                <Button className="bg-zinc-900 hover:bg-zinc-800 rounded-xl px-8 font-bold">Mua sắm ngay</Button>
              </SheetClose>
            </div>
          ) : (
            items.map((item) => (
          ...
          <div className="p-5 border-t bg-white shrink-0 space-y-4 shadow-[0_-15px_30px_-15px_rgba(0,0,0,0.05)] z-10 rounded-t-3xl">
            <div className="space-y-1.5 px-1">
                ...
                <div className="flex justify-between items-center pt-2">
                    <span className="text-sm font-black text-zinc-900 uppercase tracking-tighter">Tổng tạm tính</span>
