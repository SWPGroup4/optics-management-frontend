...
function PolicyModal({ policy, onClose, onSaved }: {
    ...
    const inputCls = "w-full px-3.5 py-2.5 text-sm border border-gray-200 rounded-xl bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-300 focus:border-indigo-400 focus:bg-white transition-all";
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center p-4">
            ...
            <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col overflow-hidden">
                ...
                <div className="overflow-y-auto flex-1 px-6 py-5 space-y-4">
                    ...
                    <div>
                        <label className={labelCls}>Mã chính sách <span className="text-rose-500">*</span></label>
                        <input name="code" value={form.code} onChange={handleChange}
                            placeholder="VD: POLICY_RETURN_30_DAYS"
                            className={inputCls + " font-mono"} />
                        <p className="text-[11px] text-gray-400 mt-1">Viết hoa, dùng dấu gạch dưới, không dấu cách</p>
                    </div>

                    <div>
                        <label className={labelCls}>Tiêu đề <span className="text-rose-500">*</span></label>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className={labelCls}>Hiệu lực từ <span className="text-rose-500">*</span></label>
                            <input type="date" name="effectiveFrom" value={form.effectiveFrom} onChange={handleChange}
                                className={inputCls} />
                        </div>
                        <div>
                            <label className={labelCls}>Hiệu lực đến <span className="text-rose-500">*</span></label>
                            <input type="date" name="effectiveTo" value={form.effectiveTo} onChange={handleChange}
                                className={inputCls} />
                        </div>
                    </div>
                </div>
...
function DeleteConfirmModal({ policy, onClose, onDeleted }: {
    policy: Policy;
    onClose: () => void;
    onDeleted: () => void;
}) {
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center p-4">
            ...
            <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 space-y-4">
                ...
                <p className="text-sm text-gray-600">
                    Hành động này <span className="font-semibold text-rose-600">không thể hoàn tác</span>. Chính sách sẽ bị xóa vĩnh viễn.
                </p>
        ...
        <div className="bg-white border border-gray-100 rounded-2xl p-5 shadow-sm hover:shadow-md transition-all duration-200 space-y-3">
            <div className="flex items-start justify-between gap-3">
                <div className="min-w-0 flex-1">
                    ...
                    <p className="text-xs text-gray-400 mt-0.5">bởi {policy.managerUsername} · #{policy.id}</p>
                </div>
                <div className="flex items-center gap-1.5 shrink-0">
                    <button onClick={onEdit}
                        className="p-2 rounded-lg text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 transition-colors" title="Chỉnh sửa">
                    ...
                    <button onClick={onDelete}
                        className="p-2 rounded-lg text-gray-400 hover:text-rose-600 hover:bg-rose-50 transition-colors" title="Xóa">
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
                                d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                        </svg>
        ...
        <div className="min-h-screen bg-[#f7f8fa]">
            ...
            {showCreate && (
                <PolicyModal
                    ...
                    onSaved={() => { fetch(0); setCurrentPage(0); showToast("Tạo chính sách thành công!", "success"); }}
                />
            )}
            ...
            {editPolicy && (
                <PolicyModal
                    ...
                    onSaved={() => { fetch(currentPage); showToast("Cập nhật thành công!", "success"); }}
                />
            )}
            ...
            {deletePolicy && (
                <DeleteConfirmModal
                    ...
                    onDeleted={() => { fetch(0); setCurrentPage(0); showToast("Đã xóa chính sách!", "success"); }}
                />
            )}
                    ...
                    <div>
                        ...
                        <p className="text-sm text-gray-500 mt-1">
                            ...
                            {(totalElements - activeCount) > 0 && (
                                <span className="text-gray-400"> · {totalElements - activeCount} không hiệu lực</span>
                            )}
                        </p>
                    </div>
                    <div className="flex items-center gap-2">
                        <button onClick={() => fetch(currentPage)}
                            className="p-2 text-gray-400 hover:text-gray-600 hover:bg-white rounded-lg border border-gray-200 transition-colors" title="Làm mới">
