            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 gap-6">
                <div>
                    <div className="flex items-center gap-2 mb-2">
                        <h1 className="text-3xl font-extrabold tracking-tight text-slate-900">Quản lý sản phẩm</h1>
                    </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-200/60 shadow-xl shadow-slate-200/40 overflow-hidden flex flex-col min-h-[600px]">
                        ...
                        <div className="relative" ref={filterRef}>
                            ...
                            {isFilterOpen && (
                                <div className="absolute top-full right-0 mt-2 w-60 bg-white border border-slate-100 rounded-xl shadow-xl z-50 p-1.5 animate-in fade-in zoom-in-95">
                                    <div className="px-3 py-2 text-xs font-bold text-slate-400 uppercase tracking-wider">Lọc theo loại</div>
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="border-b border-slate-100 bg-slate-50/50">
                                    <th className="px-6 py-4 text-sm font-bold text-slate-400 uppercase tracking-wider w-[100px]">Xem trước</th>
                                    <th className="px-6 py-4 text-sm font-bold text-slate-400 uppercase tracking-wider cursor-pointer group hover:text-indigo-600 transition-colors">
                                        <div className="flex items-center gap-1">Thông tin sản phẩm <ArrowUpDown className="w-3 h-3 opacity-0 group-hover:opacity-100" /></div>
                                    </th>
                                    <th className="px-6 py-4 text-sm font-bold text-slate-400 uppercase tracking-wider">Thông số kỹ thuật</th>
                                    <th className="px-6 py-4 text-sm font-bold text-slate-400 uppercase tracking-wider text-center">Danh mục</th>
                                    <th className="px-6 py-4 text-sm font-bold text-slate-400 uppercase tracking-wider text-center">Trạng thái</th>
                                    <th className="px-6 py-4 text-sm font-bold text-slate-400 uppercase tracking-wider text-center">Thao tác</th>
                                </tr>
                            </thead>
                                                ...
                                                <div className="flex flex-col gap-1.5 text-sm">
                                                    {product.gender && (
                                                        <div className="flex items-center gap-2 text-slate-600">
                                                            ...
                                                            <span>Đối tượng: <span className="font-semibold text-slate-900">{product.gender}</span></span>
                                                        </div>
                                                    )}
                                                    {product.frameMaterial && (
                                                        <div className="flex items-center gap-2 text-slate-600">
                                                            <span className="w-1.5 h-1.5 rounded-full bg-slate-300"></span>
                                                            <span>Chất liệu: <span className="font-semibold text-slate-900">{product.frameMaterial}</span></span>
                                                        </div>
                                                    )}
                                                    {!product.gender && !product.frameMaterial && <span className="text-sm text-slate-300 italic">Không có thông số</span>}
                                                </div>
                                            </td>
                                                        ...
                                                        {openActionId === product.id && (
                                                            <div className="absolute right-0 top-full mt-1 w-48 bg-white border border-slate-100 rounded-xl shadow-xl shadow-slate-200/50 z-50 py-1.5 text-left animate-in fade-in zoom-in-95 duration-200 origin-top-right">
                                                                <div className="px-4 py-2 border-b border-slate-50 mb-1">
                                                                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Quản lý</p>
                                                                </div>
                                                                <button
                ...
                <div className="bg-white px-6 py-5 border-t border-slate-100 flex items-center justify-between gap-4 text-sm sticky bottom-0 z-10">
                    ...
                    <div className="flex gap-2.5">
                        <button className="px-4 py-2 border border-slate-200 rounded-lg hover:bg-slate-50 text-slate-600 font-medium disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-sm" disabled>Trước đó</button>
                        <button className="px-4 py-2 border border-slate-200 rounded-lg hover:bg-slate-50 text-slate-600 font-medium disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-sm" disabled>Tiếp theo</button>
                    </div>
                </div>
            </div>
