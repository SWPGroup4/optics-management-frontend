...
const ProductVariantManagePage = () => {
  ...
  const getStockStatus = (stock: number) => {
    if (stock === 0)
      return { label: 'Out of Stock', color: 'text-red-600 bg-red-50 border-red-200 icon-red-500' };
    if (stock < 10)
      return {
        label: 'Low Stock',
        color: 'text-amber-600 bg-amber-50 border-amber-200 icon-amber-500',
      };
    return {
      label: 'In Stock',
      color: 'text-emerald-600 bg-emerald-50 border-emerald-200 icon-emerald-500',
    };
  };
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            ...
            <p className="text-slate-500 text-base max-w-2xl">
              Manage SKUs, pricing, and inventory for{' '}
              <span className="font-semibold text-slate-800">Ray-Ban Aviator Classic</span>.
            </p>
          </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200/60 shadow-xl shadow-slate-200/40 overflow-hidden flex flex-col min-h-[600px]">
        ...
        <div className="px-8 py-5 border-b border-slate-100 bg-white/50 backdrop-blur-xl sticky top-0 z-10 flex flex-col sm:flex-row justify-between items-center gap-4">
          ...
          <div className="text-sm text-slate-500 font-medium">
            Total Variants:{' '}
            <span className="text-slate-900 font-bold">{filteredVariants.length}</span>
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-slate-100 bg-slate-50/50">
                {/* HEADER ALIGNMENT */}
                <th className="px-6 py-4 text-sm font-bold text-slate-400 uppercase tracking-wider w-[80px]">
                  Image
                </th>
                    ...
                    >
                      ...
                      <td className="px-6 py-5 align-middle">
                        <div className="flex flex-col gap-1.5">
                          <span className="font-bold text-slate-800 text-[15px] group-hover:text-indigo-600 transition-colors cursor-pointer">
                            {variant.name}
                          </span>
                      ...
                      <td className="px-6 py-5 align-middle">
                        <div className="flex flex-col gap-2">
                          ...
                          <div
                            className={`flex items-center gap-1.5 w-fit px-2.5 py-1 rounded-md text-xs font-bold border ${stockInfo.color}`}
                          >
                            {variant.stock < 10 ? (
                      ...
                      <td className="px-6 py-5 align-middle text-center">
                        <span
                          className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-bold border gap-1.5 ${
