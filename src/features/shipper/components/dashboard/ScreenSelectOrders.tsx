...
export function ScreenSelectOrders({ orders, selectedIds, onToggle, onConfirm, onViewAccepted }: Props) {
    ...
    return (
        <div className="flex flex-col min-h-screen">
                            ...
                            <div className="flex items-start gap-3">
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center justify-between gap-2">
                                        ...
                                        <span className="text-xs font-medium text-muted-foreground">{order.customerId}</span>
                                    </div>
                                    <p className="mt-1 text-sm text-muted-foreground truncate">{order.deliveryAddress}</p>
            ...
            {/* Sticky footer */}
            <div className={`fixed bottom-0 ${collapsed ? 'left-16' : 'left-64'} right-0 bg-card border-t border-border p-4 safe-bottom transition-all duration-300`}>
                <button
                    type="button"
                    onClick={onConfirm}
                    disabled={count === 0}
