            ...
            <div className="flex justify-between items-start gap-3">
                <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap mb-1">
                        <span
                            className={`text-[10px] px-2 py-0.5 rounded-full font-semibold ${item.orderItemType === 'PRE_ORDER'
...
function OrderCard({ order }: { order: Order }) {
    ...
    const handleCancel = async () => {
        ...
        try {
            ...
            // Cập nhật danh sách hoàn tiền (nếu có)
            await refundApi.createBatch([order.orderId]);
                ...
                <div className="flex items-center gap-3 shrink-0 ml-2">
                    ...
                    <div className="flex items-center gap-1.5">
                        <span className="font-bold text-gray-800 text-sm">
                            {formatPrice(order.finalTotalAfterRefund)}
                        </span>
                        <svg
            ...
            {expanded && (
                <div className="border-t border-gray-100 px-5 py-4 bg-gray-50/40 space-y-4">
                    ...
                    <div className="bg-white border border-gray-100 rounded-xl px-4 py-3 space-y-1.5">
                        ...
                        {order.paidAmount >= order.finalTotalAfterRefund &&
                            order.finalTotalAfterRefund > 0 &&
                            ...
                            {order.orderStatus === 'CANCELLED' || order.orderStatus === 'REFUNDED' ? (
                                <div className="flex flex-col items-end">
                                    ...
                                    <span className="text-[10px] text-gray-400 font-medium">
                                        (Đơn hàng {order.orderStatus === 'CANCELLED' ? 'đã hủy' : 'đã hoàn tiền'})
                                    </span>
                                </div>
                ...
                {visibleStatuses.map((status) => {
                    ...
                    return (
                        <button
                            ...
                            className={`flex-shrink-0 flex items-center gap-1.5 px-4 py-2 rounded-full text-sm font-medium border transition-all ${isActive
                                    ? status === 'ALL'
                        ...
                        {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                            ...
                            return (
                                <button
                                    ...
                                    className={`w-8 h-8 text-xs font-medium rounded-lg transition-colors ${page === currentPage
