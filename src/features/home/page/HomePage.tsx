                    ...
                    <div className="max-w-7xl mx-auto px-4 md:px-8 w-full grid grid-cols-1 md:grid-cols-2 gap-8 items-center relative z-10">
                        <div className="space-y-6 md:space-y-8 animate-in slide-in-from-bottom-10 duration-1000 fade-in order-2 md:order-1 pb-10 md:pb-0">
                            ...
                            <p className="text-base md:text-lg text-gray-600 max-w-md font-light leading-relaxed">
                                Sự cân bằng hoàn hảo giữa nghệ thuật chế tác thủ công và công nghệ quang học hiện đại.
                                Nhẹ nhàng, bền bỉ và tinh tế trong từng đường nét.
                            </p>
                            <div className="pt-2">
                                <Link to="/shop">
                ...
                <section className="max-w-7xl mx-auto px-4 md:px-8 py-12">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        {[
                            ...
                            <Link to={`/shop?gender=${item.query}`} key={idx} className="group relative h-[400px] overflow-hidden rounded-[15px] block">
                ...
                <div className="max-w-7xl mx-auto px-4 md:px-8 space-y-16 md:space-y-20 py-12 md:py-24">
                    ...
                    <section>
                        ...
                        {isLoading ? (
                            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
                                {[1, 2, 3, 4].map((i) => <div key={i} className="aspect-[4/5] bg-gray-100 animate-pulse rounded-[15px]"></div>)}
                            </div>
                        ) : (
                                    ...
                                    return (
                                        <Link to={`/products/${product.id}`} key={product.id} className="group block">
                                            <div className="relative aspect-[4/5] bg-[#F9F9F9] overflow-hidden mb-4 border border-transparent group-hover:border-gray-200 transition-all rounded-[15px]">
                                                {genderBadge && (
                                                    <span className={`absolute top-3 left-3 z-10 backdrop-blur-md text-[9px] md:text-[10px] font-bold px-2 md:px-2.5 py-1 md:py-1.5 rounded-md border uppercase tracking-wider ${genderBadge.style}`}>
                                                        {genderBadge.label}
                                                    </span>
                                                )}
                                                <img
                                            ...
                                            <div className="flex flex-col">
                                                ...
                                                <p className="text-gray-400 text-[10px] uppercase tracking-wider mt-1 mb-2">{product.brand || 'Thương hiệu cao cấp'}</p>
                    ...
                    <section className="relative rounded-[15px] md:rounded-[20px] overflow-hidden min-h-[450px] md:min-h-[500px] flex flex-col justify-center">
                        ...
                        {bannerSlides.map((slide, idx) => (
                            <div
                                key={idx}
                                className={`absolute inset-0 bg-gradient-to-br ${slide.accent} transition-all duration-700 ease-in-out ${idx === currentSlide ? "opacity-100 z-10" : "opacity-0 z-0"
                                    }`}
                            >
                        ...
                        <div className="relative z-20 px-4 py-12 md:px-16 text-center text-white">
                            <div className={`max-w-3xl mx-auto space-y-4 md:space-y-6 transition-all duration-500 ${isAnimating ? "opacity-0 translate-y-4" : "opacity-100 translate-y-0"}`}>
                                <div className="flex justify-center items-center gap-2 mb-2">
                                    <Sparkles className="w-3 h-3 text-gray-400" />
                                    <span className="text-[9px] md:text-[10px] font-bold tracking-[0.3em] text-gray-400 uppercase">{bannerSlides[currentSlide].tag}</span>
                                    <Sparkles className="w-3 h-3 text-gray-400" />
                                </div>
                                ...
                                <h2 className="text-2xl sm:text-3xl md:text-5xl lg:text-6xl font-serif leading-tight">
                                    ...
                                    <span className="text-gray-400 italic font-light sm:ml-2">{bannerSlides[currentSlide].subline}</span>
                                </h2>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-w-lg mx-auto pt-4">
                                    {bannerSlides[currentSlide].badges.map((badge, bIdx) => (
                                        <div key={bIdx} className="flex items-center justify-center gap-3 bg-white/5 rounded-[12px] p-3 border border-white/10">
                                            {badge.icon === "shield" ? <ShieldCheck className="w-4 h-4 text-gray-300" /> : <Gem className="w-4 h-4 text-gray-300" />}
                                            <span className="text-[10px] md:text-xs font-medium uppercase text-gray-300">{badge.label}</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
                            ...
                            {/* Navigation */}
                            <button onClick={handlePrevSlide} className="absolute left-3 md:left-6 top-1/2 -translate-y-1/2 z-30 w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 border border-white/10 flex items-center justify-center backdrop-blur-sm transition-all">
                                <ChevronLeft className="w-5 h-5 text-white" />
                            </button>
                            <button onClick={handleNextSlide} className="absolute right-3 md:right-6 top-1/2 -translate-y-1/2 z-30 w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 border border-white/10 flex items-center justify-center backdrop-blur-sm transition-all">
