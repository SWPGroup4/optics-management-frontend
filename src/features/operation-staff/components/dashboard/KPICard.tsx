...
const KPICard: React.FC<KPICardProps> = ({ data }) => {
    ...
    return (
        <div className={`bg-white dark:bg-[#1a262d] p-5 rounded-xl border ${styles.border} shadow-sm flex flex-col justify-between h-32 relative overflow-hidden group`}>
            <div className={`absolute right-0 top-0 p-4 opacity-40 group-hover:opacity-80 transition-opacity ${styles.icon}`}>
            ...
            <div className="w-full bg-slate-100 dark:bg-slate-700 h-1 mt-2 rounded-full overflow-hidden">
                <div className={`${styles.progress} h-full rounded-full`} style={{ width: `${data.percentage}%` }} />
            </div>
        </div>
    );
};
